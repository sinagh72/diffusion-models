...
from models.cls import Classifier, LSTMClassifier, LSTMAttentionClassifier
...
if __name__ == "__main__":
    ...
    batch_size_pt = 24
    batch_size_ft = 32
    num_workers = torch.cuda.device_count() * 16
    epochs = 100
    pt_comments = "semantic_diffusion_12"
    ft_comments = "ft_semantic_diffusion_12"
    # devices = [0, 1]
    devices = [0, 1, 2, 3]
    img_size = 128
    resume_pt = True
    ...
    combined_train_list = (
            ...
            data_modules[9].dataset.img_paths +
            data_modules[10].dataset.img_paths 
            )
    train_dataset = OCTDataset(transform=get_train_transformation(img_size, channels=1, mean=[0.5], std=[0.5]),
                               data_dir="",
    ...
    val_dataset = OCTDataset(transform=get_test_transformation(img_size, channels=1, mean=[0.5], std=[0.5]),
                             data_dir="",
    ...
    trainer = pl.Trainer(accelerator='gpu', devices=devices, max_epochs=epochs,
                        ...
                        # strategy = FSDPStrategy(process_group_backend='gloo'),
                        #  strategy=DeepSpeedStrategy(process_group_backend='gloo', stage=2),
                        #  precision="16-mixed",
    ...
    # cls_model = LSTMAttentionClassifier(encoder=copy.deepcopy(diffusion),
    #                        feature_dim=1000,
